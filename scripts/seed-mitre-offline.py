#!/usr/bin/env python3
"""Seed MITRE ATT&CK techniques into Qdrant from a local STIX2 file (air-gapped fallback)."""
...
def load_local_stix(file_path: str) -> dict:
    logger.info("Loading local STIX2 file: %s", file_path)
    if not os.path.isfile(file_path):
        raise FileNotFoundError(f"STIX2 file not found: {file_path}")
    with open(file_path, "r", encoding="utf-8") as f:
        data = json.load(f)
    logger.info("Loaded %.1f MB", os.path.getsize(file_path) / (1024 * 1024))
    return data
...
def main():
    parser = argparse.ArgumentParser(description="Seed MITRE ATT&CK from local STIX2 file")
    parser.add_argument("--file", required=True, help="Path to local enterprise-attack.json STIX2 file")
    ...
    stix_data = load_local_stix(args.file)
